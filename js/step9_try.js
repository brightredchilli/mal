      ...
      switch (String(ast[0])) {
        case "try*": {
          ...
          try {
            ...
            let [__, argName, handler] = second
            // console.log(`catch?(${__}), argName=${argName} exc=${exc}`)
            let newEnv = new Env(env, [argName], [exc])
            return EVAL(handler, newEnv)
          }
        }
        case "defmacro!":
        case "def!": {
...
function eval_ast(ast, env) {
  if (ast instanceof Array) {
    ...
    let mapped = new Map()
    for (let key of ast.keys()) {
      mapped.set(key, EVAL(ast.get(key), env))
    }
    return mapped
